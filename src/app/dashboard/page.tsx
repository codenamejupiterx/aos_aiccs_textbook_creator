...
export default function DashboardPage() {
  ...
  return (
    <div className="h-screen grid grid-rows-[auto,1fr,auto] bg-black text-white">
      {/* Header */}
      <header className="w-full bg-gradient-to-r from-pink-500 via-indigo-500 to-cyan-400 shadow-lg">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
          <h1 className="flex items-center gap-3 text-xl sm:text-2xl font-extrabold tracking-wide drop-shadow-lg">
            <span>AOS Artificial Intelligent Custom Curriculum System</span>
          </h1>

          <button
            type="button"
            onClick={() => setPanelOpen(true)}
            className="w-[220px] select-none rounded-2xl bg-emerald-500 px-6 py-3 text-white font-semibold shadow-lg shadow-emerald-900/20 ring-1 ring-emerald-400/20 hover:bg-emerald-600 active:bg-emerald-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-emerald-400 transition"
          >
            My Mini-Chapters
          </button>
        </div>
            ...
            <div className="rounded-3xl border border-white/10 bg-[#222833] shadow-[0_20px_60px_-30px_rgba(0,0,0,0.8)] py-8 px-6 sm:px-8">
              <div className="max-w-3xl mx-auto w-full">
                <form id="aiccsForm" className="space-y-6" onSubmit={handleSubmit}>
                  ...
                  <div>
                    <label className={label}>List 5–10 things you love</label>
                    <textarea
                      rows={4}
                      className={field + " resize-none"}
                      value={loves}
      ...
      {/* Footer */}
      <footer className="w-full bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 py-3 shadow-inner">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 flex justify-between items-center text-xs sm:text-sm">
          <span />
          <div className="hover:text-yellow-300">
            © {new Date().getFullYear()} Allure of STEM
          </div>
        </div>
      </footer>
      ...
      {loading && (
        <div className="fixed inset-0 z-[9999] grid place-items-center bg-black/60 backdrop-blur-sm">
          <div className="rounded-2xl bg-[#0f1216] p-6 shadow-xl border border-white/10 text-center">
            {/* Place a GIF at /public/processing.gif */}
            <img
              src="/processing.gif"
              alt="Processing…"
        ...
        <div className="fixed inset-0 z-[9999] grid place-items-center bg-black/60 backdrop-blur-sm">
          <div className="w-[min(92vw,520px)] rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="mt-5 flex gap-3">
              <button
                onClick={() => {
                  setPanelOpen(true);  // open slideout
                  setSuccessId(null);  // close modal
                }}
